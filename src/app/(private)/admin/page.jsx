      ...
      <div className="flex flex-col items-center gap-3 mt-4">
        ...
        {myEvents && myEvents.length > 0 ? (
          myEvents.map((event) => (
            <div
              key={event.id}
              className="p-3 rounded-lg shadow-sm flex flex-col gap-2 text-black w-full max-w-md"
            >
              <Link href={`/admin/detail?eventId=${event.id}`}>
                <h2 className="text-lg font-semibold active:opacity-80 hover:underline">
                  {event.name}
                </h2>
              </Link>
