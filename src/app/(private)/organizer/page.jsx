...
const StatusModal = ({ message, type, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50 p-4">
      <div className="rounded-xl shadow-lg p-5 w-full max-w-sm transform transition-all scale-100 text-gray-800 bg-white">
        <h3
          className={`text-2xl font-bold mb-4 border-b border-gray-800 border-opacity-30 pb-2 ${textColor}`}
        >
          {title}
        </h3>
...
export default function Page() {
  ...
  const API_URL =
    "https://capsular-peddlingly-immanuel.ngrok-free.dev/webhook/post/attend";
  ...
  const sendToServer = async (text) => {
    ...
    try {
      ...
      if (result && result.success === true) {
        ...
        throw new Error(
          result.data || result.message || "Verifikasi tiket gagal."
        );
      }
    } catch (err) {
  ...
  useEffect(() => {
    if (status === "unauthenticated") {
      ...
      if (session !== undefined && session?.user.role !== "committee") {
        router.push("/auth");
        router.refresh();
      }
    }
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col items-center p-4 sm:p-8">
      <button
        ...
        className="flex flex-row gap-2 text-white font-medium py-2 px-4 rounded-md bg-red-500 self-start my-6 hover:bg-red-400 active:opacity-80 transition duration-100 disabled:bg-gray-500"
      >
        ...
        {!isScanning ? (
          <div className="flex flex-col items-center gap-6">
            ...
            <p className="text-center text-gray-600 font-medium">
              Siap untuk memindai? Klik tombol di bawah untuk mengaktifkan
              kamera dan memverifikasi tiket.
            </p>
            <button
          ...
          <div className="flex flex-col items-center gap-4">
            <div
              id="reader"
              className="w-full h-auto max-w-xs sm:max-w-none"
            ></div>
          ...
          {scanResult && !loading && (
            <div className="mt-5 p-4 bg-purple-50 border-l-4 border-purple-500 rounded-md text-gray-800">
              <strong className="text-purple-700">
                ID Tiket Terakhir Diverifikasi:
              </strong>
              <p className="mt-1 font-mono wrap-break-word text-sm">
                {scanResult}
              </p>
            </div>
          )}
