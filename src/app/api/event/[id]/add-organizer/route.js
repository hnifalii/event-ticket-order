...
console.log("API call hit: /api/event/[id]/add-organizer");
let cacheCount = 0;
...
export async function POST(request, { params }) {
  ...
  try {
    ...
    const userId = userRows.length + 1;
    const eventOrganizerId = eventOrganizerRows.length + 1;
    ...
    const registerResponse = await sheets.spreadsheets.values.append({
      spreadsheetId: process.env.SPREADSHEET_ID,
      range: "users!A2:F",
      valueInputOption: "USER_ENTERED",
      requestBody: {
        values: [
          [userId, body.username, body.password, body.email, body.name, body.role],
        ],
      },
    });
    ...
    const response = await sheets.spreadsheets.values.append({
      ...
      requestBody: {
        values: [
          [eventOrganizerId, id, userId, body.name],
        ],
      },
    });
